...
class YahtzeeBoard:
    ...
    def initInterface(self):  # Yahtzee 보드 윈도우 생성
        ...
        self.TempFont = tkinter.font.Font(size=16, weight='bold', family ='Consolas')
        for I in range(5):  # Dice 객체 5개 생성
            self.dice.append(Dice())
        self.rollDice = Button(self.window, text="Roll Dice", font = self.TempFont,bg='white',
                      command = self.rollDiceListener) # RollDice버튼
        self.rollDice.grid(row=0, column=0)
        for i in range(5):  # dice 버튼 5개 생성
            self.diceButtons.append(Button(self.window, text="?",bg='white',
        ...
        for i in range(self.TOTAL + 2):  # i행 : 점수
            ...
            for j in range(self.numPlayers):  # j열 : 플레이어
                if (i == 0):  # 플레이어 이름 표시
                    ...
                    self.fields[i - 1].append(Button(self.window, text="", font=self.TempFont, width=8,bg='white',
