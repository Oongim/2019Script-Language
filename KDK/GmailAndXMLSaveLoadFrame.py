...
class GmailAndXMLSaveLoadFrame(Frame):
    def __init__(self, mainframe, window,viewport):
        ...
        tkinter.Label(window, font=TempFont, text="목록 내보내기 ").pack(side=tkinter.LEFT)

        setframe = tkinter.Frame(window)
        setframe.pack(side=tkinter.LEFT)
        gmailframe=tkinter.Frame(setframe)
        gmailframe.pack()
        TempFont = tkinter.font.Font(window, size=15, weight='bold', family='Consolas')
